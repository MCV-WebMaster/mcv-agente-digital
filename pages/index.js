...
export default function SearchPage() {
  ...
  const [filters, setFilters] = useState({
    ...
    pets: false,
    pool: false,
    barrio: '', // El barrio ahora se usará
  });

  // --- 2. ESTADO DE LOS RESULTADOS Y BARRIOS ---
  const [results, setResults] = useState([]);
  const [barrios, setBarrios] = useState([]); // ¡NUEVO! Para guardar la lista de barrios
  ...
  const fetchProperties = useCallback(async () => {
    ...
    if (filters.barrio) params.append('barrio', filters.barrio); // ¡AHORA SE APLICA!
  ...
  // (Se añadió filters.barrio a las dependencias de useCallback)


  // --- 4. CARGAR BARRIOS (¡NUEVO!) ---
  // Este useEffect se ejecuta 1 sola vez al cargar la página
  useEffect(() => {
    async function loadBarrios() {
      try {
        const res = await fetch('/api/get-barrios');
  ...
  // useEffect "vigila" los filtros (sin cambios)
  useEffect(() => {
  ...
  // --- 5. MANEJADORES DE EVENTOS (sin cambios) ---
          ...
          <aside className="w-full md:w-1/4 p-4 bg-gray-800 rounded-lg shadow-lg h-fit">
            ...
            <div className="mb-4">
              ...
              >
                ...
                <option value="Venta" disabled>Venta (próximamente)</option>
                <option value="Alquiler" disabled>Alquiler (próximamente)</option>
              </select>
            </div>
            ...
            {filters.type === 'Alquiler Temporal' && (
              <div className="space-y-4">
                ...
                {/* --- FILTRO DE BARRIOS (¡NUEVO!) --- */}
                <div>
                  <label htmlFor="barrio" className="block text-sm font-medium text-gray-300">Barrio</label>
                  <select
                    id="barrio"
                    name="barrio"
