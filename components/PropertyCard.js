    ...
    <div className="bg-white rounded-lg shadow-md overflow-hidden hover:shadow-xl transition-shadow duration-300 border border-gray-200 flex flex-col h-full group">
      ...
      <div className="p-5 flex flex-col flex-grow">
            ...
            <div className="flex gap-4">
                {property.bedrooms > 0 && (
                    <div className="flex items-center gap-1" title="Dormitorios">
                        <FaBed className="text-gray-400" /> <span className="font-semibold">{property.bedrooms}</span>
                    </div>
                )}
                {property.bathrooms > 0 && (
                    <div className="flex items-center gap-1" title="Baños">
                        <FaBath className="text-gray-400" /> <span className="font-semibold">{property.bathrooms}</span>
                    </div>
                )}
                {property.mts_cubiertos > 0 && (
                    <div className="flex items-center gap-1" title="Metros Cubiertos">
                        <FaRulerCombined className="text-gray-400" /> <span className="font-semibold">{property.mts_cubiertos}m²</span>
                    </div>
                )}
            </div>
        </div>
        ...
        <div className="grid grid-cols-2 gap-3 mt-5">
           <a href={property.url} target="_blank" rel="noopener noreferrer" className="flex items-center justify-center w-full py-2.5 border-2 border-gray-200 text-gray-700 font-bold rounded-lg hover:border-mcv-azul hover:text-mcv-azul transition-colors text-sm">Ver Ficha</a>
           <button onClick={() => onContact(property)} className="flex items-center justify-center w-full py-2.5 bg-mcv-verde text-white font-bold rounded-lg hover:bg-opacity-90 transition-colors shadow-sm text-sm"><FaWhatsapp className="mr-2 text-lg" /> Consultar</button>
        </div>
      </div>
    </div>
  );
