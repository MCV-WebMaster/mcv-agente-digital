...
// --- DICCIONARIO DE TRADUCCIÓN (CRÍTICO - SINCRONIZADO CON index.js) ---
const PERIOD_MAPPING = {
  'Navidad': 'navidad',
  'Año Nuevo': 'ano-nuevo',
  'Enero Combinado': 'enero-1ra-quincena', // ESTE ES EL FIX: Nombre en el front mapea a slug de DB
...
export async function searchProperties(filters) {
  ...
  if (operacion === 'alquiler_temporal') {
    ...
    // Se obtiene el slug a partir del nombre enviado por el frontend.
    ...
    for (const period of allPeriodsData || []) {
        ...
        // --- LÓGICA DE COINCIDENCIA DE PERIODO (usa includes para robustez) ---
        let isMatch = false;
        if (targetPeriodSlug) {
            const dbName = period.period_name ? period.period_name.toLowerCase() : '';
            
            // Check si el slug de la DB contiene el slug que buscamos.
