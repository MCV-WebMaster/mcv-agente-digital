...
// Conecta la selección visual con el slug real de la base de datos
const PERIOD_MAPPING = {
  ...
  'Febrero 2da Quincena': 'diciembre-2da-quincena' // Según tu lista histórica
};
...
// Parser para leer precios escondidos en notas (Vital para GBA)
...
export async function searchProperties(filters) {
  ...
  if (operacion === 'alquiler_temporal') {
    ...
    // Traducción del periodo usando el Diccionario
    ...
    for (const period of allPeriodsData || []) {
        ...
        // Lógica de Coincidencia (Inclusiva)
        let isMatch = false;
        if (targetPeriodSlug) {
    ...
    let finalResults = propertiesData.map(p => ({
        ...
        // 2. Filtro Precio (Solo si el usuario lo pidió)
        const priceToCheck = (targetPeriodSlug && p.found_period_price) ? p.found_period_price : p.min_rental_price;
        
        // Si el precio es 0/null (Consultar) y el usuario NO puso filtro de precio, la mostramos.
        // Si el usuario puso filtro (ej: min 1000), las de precio 0 se ocultan (correcto).
  ...
  // --- VENTA / ANUAL ---
    ...
    // EXCEPCIÓN LOTE VENTA: No filtrar por dormitorios
    if (bedrooms && tipo !== 'lote') {
        const bedroomsNum = parseInt(bedrooms, 10);
        query = bedrooms_or_more ? query.gte('bedrooms', bedroomsNum) : query.eq('bedrooms', bedroomsNum);
    }
