...
// Parser para leer precios escondidos en el texto (notas)
...
export async function searchProperties(filters) {
  ...
  if (operacion === 'alquiler_temporal') {
    ...
    // Lógica de Categorías Estrictas
    if (showOtherDates) {
    ...
    // Filtro de Dormitorios: SOLO si NO es lote. Si es Lote, ignoramos dormitorios.
    ...
    // Filtro de Pax: SOLO si NO es lote.
    if (pax && tipo !== 'lote') {
    ...
    const userSelectedPeriod = selectedPeriod ? selectedPeriod.trim().toLowerCase() : null;
    ...
    for (const period of allPeriodsData || []) {
        ...
        // --- FIX DE COINCIDENCIA DE PERIODO ---
        let isMatch = false;
        if (userSelectedPeriod) {
            const dbPeriodName = period.period_name ? period.period_name.trim().toLowerCase() : '';
            
            // Comparación:
            // 1. Exacta (normalizada)
            if (dbPeriodName === userSelectedPeriod) {
                isMatch = true;
            } 
            // 2. Parcial segura (para casos como "Año Nuevo c/1ra" vs "Año Nuevo con 1ra")
            // Verificamos si ambas cadenas contienen palabras clave críticas
            else if (userSelectedPeriod.includes('año nuevo') && userSelectedPeriod.includes('1ra')) {
                 if (dbPeriodName.includes('año nuevo') && dbPeriodName.includes('1ra')) {
                     isMatch = true;
                 }
            }
        } else {
            isMatch = true; // Si no eligió periodo específico, machea todo lo disponible
        }
    ...
    let finalResults = propertiesData.map(p => ({
        ...
        // Filtro estricto de disponibilidad para temporada alta
  ...
  // --- VENTA / ANUAL ---
    ...
    // FIX LOTE: Ignorar dormitorios si es lote
    if (bedrooms && tipo !== 'lote') {
        const bedroomsNum = parseInt(bedrooms, 10);
        if (bedrooms_or_more === true) {
            query = query.gte('bedrooms', bedroomsNum);
