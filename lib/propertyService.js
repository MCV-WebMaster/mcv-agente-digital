...
// --- DICCIONARIO DE TRADUCCIÓN (CLAVE PARA EL FIX) ---
// Key: Valor del Frontend. Value: Nombre exacto en la columna periods.period_name.
const PERIOD_MAPPING = {
  'Navidad': 'Navidad del 19 al 26/12/25', 
  'Año Nuevo': 'Año Nuevo 26/12/25 al 2/1/26', 
  'Enero Combinado': 'Año Nuevo c/1er q de enero del 30/12/25 al 15/1/26', // FIX: Usando el texto largo exacto de tu DB
  'Enero 1ra Quincena': 'Ene 1er q del 2 al 15/1/26',
  'Enero 2da Quincena': 'Ene 2da q del 16 al 31/1/26',
  'Febrero 1ra Quincena': 'Feb 1er q c/CARNAVAL del 1 al 17/2/26', 
  'Febrero 2da Quincena': 'Feb 2da q del 18/2/26 al 1/3/26'
};
...
// --- FUNCIÓN DE NORMALIZACIÓN (GARANTÍA CONTRA ESPACIOS/TILDES) ---
function normalizeString(str) {
    if (!str) return '';
    // Normalización estricta: quita tildes, espacios, pasa a minúsculas
    return str.trim().toLowerCase()
              .replace(/\s+/g, ' ') // Consolida espacios
...
export async function searchProperties(filters) {
  ...
  if (operacion === 'alquiler_temporal') {
    ...
    // Obtención del nombre EXACTO que debería tener en la DB, normalizado
    const targetPeriodDbName = selectedPeriod ? PERIOD_MAPPING[selectedPeriod] : null;
    const normalizedTargetName = normalizeString(targetPeriodDbName);
    ...
    for (const period of allPeriodsData || []) {
        ...
        // --- LÓGICA DE COINCIDENCIA CRÍTICA (USANDO NORMALIZACIÓN) ---
        let isMatch = false;
        if (targetPeriodDbName) {
            // Normalizamos el nombre de la DB para la comparación
            const normalizedDbName = normalizeString(period.period_name); 
            
            // Comparamos sin ruido (espacios, mayúsculas, tildes)
            if (normalizedDbName === normalizedTargetName) {
                isMatch = true;
            }
        } else {
            isMatch = true;
    ...
    let finalResults = propertiesData.map(p => ({
        ...
        if (shouldCheckAvailability && targetPeriodDbName) {
            if (!availablePropertyIds.has(p.property_id)) return false;
        }

        // 2. Filtro Precio
        const priceToCheck = (targetPeriodDbName && p.found_period_price) ? p.found_period_price : p.min_rental_price;
