...
export async function searchProperties(filters) {
  ...
  if (operacion === 'alquiler_temporal') {
    ...
    // Filtro de Dormitorios (Solo si NO es lote)
    ...
    for (const period of allPeriodsData || []) {
        ...
        // Lógica de Coincidencia de Periodo (Normalizada)
        let isMatch = false;
        if (userSelectedPeriod) {
            // Normalizamos strings para evitar errores de espacios o mayúsculas sutiles
            const pName = period.period_name ? period.period_name.trim().toLowerCase() : '';
            const uName = userSelectedPeriod.trim().toLowerCase();
            
            // Coincidencia exacta o parcial segura (para Año Nuevo c/1ra)
            if (pName === uName) {
                isMatch = true;
            } else if (uName.includes('año nuevo') && uName.includes('1ra') && pName.includes('año nuevo') && pName.includes('1ra')) {
                // Fix específico para variaciones de "Año Nuevo con 1ra Enero"
                isMatch = true;
            }
        } else {
            isMatch = true;
  ...
  // --- VENTA / ANUAL ---
    ...
    // Dormitorios (Solo si NO es lote)
    if (bedrooms && tipo !== 'lote') {
        const bedroomsNum = parseInt(bedrooms, 10);
        if (bedrooms_or_more === true) {
            query = query.gte('bedrooms', bedroomsNum);
