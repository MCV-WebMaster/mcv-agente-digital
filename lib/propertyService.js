...
// --- DICCIONARIO DE TRADUCCIÓN (CRÍTICO - USA TEXTO LARGO DE DB) ---
const PERIOD_MAPPING = {
  'Navidad': 'Navidad del 19 al 26/12/25', 
  'Año Nuevo': 'Año Nuevo 26/12/25 al 2/1/26', 
  'Enero Combinado': 'Año Nuevo c/1er q de enero del 30/12/25 al 15/1/26', // FIX CRÍTICO
...
// --- FUNCIÓN DE NORMALIZACIÓN CRÍTICA (GARANTÍA DE MATCH) ---
function normalizeString(str) {
    if (!str) return '';
    // 1. Quitar espacios iniciales/finales
    // 2. Pasar a minúsculas
    // 3. Quitar todos los espacios, tildes y caracteres que puedan causar fallo de comparación
...
export async function searchProperties(filters) {
  ...
  if (operacion === 'alquiler_temporal') {
    ...
    // Obtener el nombre esperado de la DB, NORMALIZADO.
    const targetPeriodDbName = selectedPeriod ? PERIOD_MAPPING[selectedPeriod] : null;
    const normalizedTargetName = normalizeString(targetPeriodDbName); // <-- NORMALIZACIÓN DEL TARGET
    ...
    for (const period of allPeriodsData || []) {
        ...
        if (targetPeriodDbName) {
            const normalizedDbName = normalizeString(period.period_name); // <-- NORMALIZACIÓN DE LA DB
            
            // Comparamos AMBOS strings sin tildes, espacios, ni mayúsculas.
