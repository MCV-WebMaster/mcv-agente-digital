...
// Recuperado: Función vital para GBA Sur donde el precio está en notas
...
export async function searchProperties(filters) {
  ...
  if (operacion === 'alquiler_temporal') {
    ...
    const { data: allPeriodsData, error: allPeriodsError } = await supabase
      .from('periods')
      .select('*') // Traemos todo para analizar
    ...
    for (const period of allPeriodsData || []) {
        ...
        // Lógica de Coincidencia de Periodo
        let isMatch = false;
        if (userSelectedPeriod) {
            // Coincidencia exacta por nombre de quincena
            if (period.period_name === userSelectedPeriod) isMatch = true;
        } else {
            // Si no hay periodo seleccionado, cualquier disponible cuenta
            isMatch = true;
        }

        if (isMatch && periodPrice > 0) {
            availablePropertyIds.add(period.property_id);
            // Guardamos el detalle específico de este periodo para mostrarlo
  ...
  // --- VENTA / ALQUILER ANUAL ---
    ...
    if (operacion === 'venta') {
      ...
      // Usamos price_ars pero podría ser price_note
    }
    ...
    // Procesamiento de precios especiales (GBA Sur)
    let processedData = data.map(p => {
        let finalPrice = p.price;
        if (operacion === 'alquiler_anual') {
            finalPrice = p.es_property_price_ars;
            // Si no hay precio numérico, intentamos leer la nota (Punto B)
    ...
    // Filtro manual de precio si usamos la nota parseada
    if (operacion === 'alquiler_anual' && (minPrice || maxPrice)) {
