...
// --- DICCIONARIO DE TRADUCCIÓN (CRÍTICO - USA NOMBRES LARGOS DE DB) ---
// Key: Valor que viene del Select. Value: Nombre exacto en la columna periods.period_name.
const PERIOD_MAPPING = {
  'Navidad': 'Navidad', 
  'Año Nuevo': 'Año Nuevo', 
  'Enero Combinado': 'Año Nuevo c/1er q de enero del 30/12/25 al 15/1/26', // FIX CRÍTICO
  'Enero 1ra Quincena': 'Enero 1ra Quincena', 
...
export async function searchProperties(filters) {
  ...
  if (operacion === 'alquiler_temporal') {
    ...
    // Obtención del nombre a buscar en la DB (Texto largo)
    ...
    for (const period of allPeriodsData || []) {
        ...
        // --- LÓGICA DE COINCIDENCIA CRÍTICA: Comparación de texto normalizado ---
        let isMatch = false;
        if (targetPeriodDbName) {
            const dbName = period.period_name ? period.period_name.trim().toLowerCase() : '';
            const targetName = targetPeriodDbName.trim().toLowerCase();
            
            // Comparamos el nombre completo de la DB contra el nombre completo esperado
