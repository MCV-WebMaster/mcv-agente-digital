...
export async function searchProperties(filters) {
  const { 
    ...
    sortBy = 'default',
    searchText,
    showOtherDates // Nuevo flag para la IA
  } = filters;
  ...
  if (operacion === 'alquiler_temporal') {
    ...
    // LÓGICA A (Igual que en la Web):
    if (showOtherDates) {
        // Si la IA determina que es "fuera de temporada", busca Cat 196
        query = query.contains('category_ids', [CATEGORY_IDS.ALQUILER_TEMPORAL]);
    } else if (isSearchingInHighSeason || !hasDateSelected) {
        // Por defecto o Temporada Alta -> Prioridad Verano (197)
    ...
    // Disponibilidad Estricta (Solo si NO es "Otras Fechas")
    ...
    if (shouldCheckAvailability && (userSelectedPeriod || hasDateSelected)) {
