...
// --- DICCIONARIO DE TRADUCCIÓN (MAPEO CRÍTICO) ---
// Clave: Lo que viene del Select/Chat. Valor: Lo que está en la DB.
const PERIOD_MAPPING = {
  'Navidad': 'navidad',
  'Año Nuevo': 'ano-nuevo',
  'Año Nuevo con 1ra Enero': 'enero-1ra-quincena', // Tu regla específica
  'Enero 1ra Quincena': 'enero-2da-quincena',      // Tu regla específica
  'Enero 2da Quincena': 'febrero-1ra-quincena',    // Tu regla específica
  'Febrero 1ra Quincena': 'febrero-2da-quincena',  // Tu regla específica
  'Febrero 2da Quincena': 'diciembre-2da-quincena',// Tu regla específica (revisar si es correcto, puesto según tu lista)
  'Diciembre 2da Quincena': 'diciembre-2da-quincena' // Asumo
};
...
export async function searchProperties(filters) {
  ...
  if (operacion === 'alquiler_temporal') {
    ...
    // TRADUCCIÓN DEL PERIODO SELECCIONADO AL SLUG DE LA DB
    ...
    for (const period of allPeriodsData || []) {
        ...
        // Lógica de Coincidencia usando el SLUG traducido
        let isMatch = false;
        if (targetPeriodSlug) {
            // Comparamos el slug esperado con el nombre en la DB (o parte de él)
            // Usamos includes para ser flexibles si la DB dice "febrero-1ra-quincena-2026"
            if (period.period_name && period.period_name.toLowerCase().includes(targetPeriodSlug.toLowerCase())) {
                isMatch = true;
            }
        } else {
            isMatch = true;
        ...
        if (isMatch) {
            ...
            // Guardamos el precio específico de ESTE periodo si existe
            if (periodPrice > 0) {
    ...
    let finalResults = propertiesData.map(p => ({
        ...
        // Si eligió periodo, SOLO mostrar si está disponible en ese periodo (está en el Set)
