...
// --- DICCIONARIO DE TRADUCCIÓN (TEXTO FRONT -> SLUG DB) ---
// Basado estrictamente en tu lista de periodos
const PERIOD_MAPPING = {
  ...
  'Febrero 1ra Quincena': 'febrero-2da-quincena', // Incluye Carnaval
  'Febrero 2da Quincena': 'diciembre-2da-quincena' // Asumido según tu lista desfasada
};
...
export async function searchProperties(filters) {
  ...
  if (operacion === 'alquiler_temporal') {
    ...
    // Ignorar dormitorios/pax si es Lote
    ...
    // Traducción del periodo seleccionado
    ...
    for (const period of allPeriodsData || []) {
        ...
        // --- CORRECCIÓN CRÍTICA: ---
        // Si hay coincidencia de periodo, lo agregamos AUNQUE el precio sea 0.
        if (isMatch) {
            availablePropertyIds.add(period.property_id);
            // Guardamos detalle solo para visualización
    ...
    let finalResults = propertiesData.map(p => ({
        ...
        // Filtro de Precio (Solo si el usuario puso min/max)
        // Si el precio es 0 (Consultar), y el usuario no puso filtro, pasa.
