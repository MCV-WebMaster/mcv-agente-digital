  ...
  if (operacion === 'alquiler_temporal') {
    ...
    let finalResults = propertiesData
      ...
      .filter(p => { 
          ...
          // Precio a filtrar: Si hay periodo, el del periodo. Si no, el "Desde".
          const priceToFilter = userSelectedPeriod ? p.found_period_price : p.min_rental_price;
          
          if (!minPrice && !maxPrice) return true;
          if (!priceToFilter) return false; // Si no hay precio, no pasa el filtro
          
          // FIX DE COMPARACIÓN NUMÉRICA
          const minVal = minPrice ? parseFloat(minPrice) : 0;
          const maxVal = maxPrice ? parseFloat(maxPrice) : Infinity;
          
          const passesMin = priceToFilter >= minVal;
          const passesMax = priceToFilter <= maxVal;
