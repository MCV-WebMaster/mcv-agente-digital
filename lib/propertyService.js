...
// --- DICCIONARIO DE TRADUCCIÓN (CRÍTICO - MAPEO STRING COMPLETO) ---
// La clave es el valor del Select (Frontend), el valor es el texto EXACTO de la DB.
...
export async function searchProperties(filters) {
  ...
  if (operacion === 'alquiler_temporal') {
    ...
    // Se obtiene el TEXTO LARGO (Database String) para la comparación
    const targetPeriodText = selectedPeriod ? PERIOD_MAPPING[selectedPeriod] : null;
    ...
    for (const period of allPeriodsData || []) {
        ...
        // --- LÓGICA DE COINCIDENCIA CRÍTICA ---
        let isMatch = false;
        if (targetPeriodText) {
            const dbName = period.period_name ? period.period_name.trim().toLowerCase() : '';
            const targetName = targetPeriodText.trim().toLowerCase();
            
            // Comparamos el texto del filtro con el texto largo de la DB
    ...
    let finalResults = propertiesData.map(p => ({
        ...
        if (shouldCheckAvailability && targetPeriodText) {
            if (!availablePropertyIds.has(p.property_id)) return false;
        }

        // 2. Filtro Precio
        const priceToCheck = (targetPeriodText && p.found_period_price) ? p.found_period_price : p.min_rental_price;
