 ...
 * Parsea el JSON del "Puente PHP" (v4) y lo divide para las tablas 'properties' y 'periods'.
 * Basado en el mapeo de datos de Estatik (IDs de Categoría, meta_keys, etc.).
 */

// --- MAPEO DE DISPONIBILIDAD ---
// (Basado en Tarea 2.25)
const PERIOD_MAP = [
...
export function processFullSyncData(allProperties) {
  ...
  for (const prop of allProperties) {
    ...
    const propertyRecord = {
      ...
      mts_terreno: parseInt(prop.area, 10) || null, // Usamos 'area' para mts de terreno
    };
    ...
    // (Solo si la propiedad es Alquiler Temporal, ID 196 o 197)
    if (prop.category_ids.includes(196) || prop.category_ids.includes(197)) {
